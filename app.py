...
def calculate_tar(player, year):
    ...
    # Position-adjusted league averages
    ...
    threepar_avg = league_avg(df[pos_filter]["3PAr"]) if '3PAr' in df.columns else 0.2  # default
    ftr_avg = league_avg(df[pos_filter]["FTr"]) if 'FTr' in df.columns else 0.2

    # Offensive factors (weighted additive)
    ...
    shooting_factor = (1 + ((p.get("3PAr", 0)/threepar_avg) + (p.get("FTr",0)/ftr_avg))/2)

    AOR = 0.25*ts_factor + 0.25*scoring_factor + 0.15*orb_factor + 0.15*tov_factor + 0.2*creation_factor
    AOR *= shooting_factor  # apply shooting boost

    # Defensive factors (weighted additive)
    drtg_factor = drtg_avg / p["DRtg"]
    drb_factor = p["DRB"] / drb_avg
    stl_factor = p["STL"] / stl_avg
    blk_factor = p["BLK"] / blk_avg
    ADR = 0.4*drtg_factor + 0.2*drb_factor + 0.2*stl_factor + 0.2*blk_factor
